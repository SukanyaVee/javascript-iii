...
let totalPopulation  = populations.reduce(function(total,val,ind,arr)
{
    return total+=val;
},0)
...
let ordersTotal = orders.map((elem,ind,arr) => elem.price*(1+elem.tax))
...
let bobsTotal = purchases.reduce((total,elem,ind,arr)=>
{
    if (elem.owner==="Bob")
    {
        total+=elem.price
